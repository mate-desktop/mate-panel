  ...
  const char *dir)
{
 ...
 path = g_build_filename (dir, basename, ((void*)0));
    ...
    const char *dir)
{
 ...
 path = g_build_filename (dir, "applications", basename, ((void*)0));
 if (!g_file_test (path, G_FILE_TEST_EXISTS)) {
  g_free (path);
  return ((void*)0);
 }
 ...
 *out = g_utf8_get_char (text);
 return (*out == (gunichar)-1) ? ((void*)0) : ((text) + g_utf8_skip[*(const guchar *)(text)]);
}
...
panel_g_utf8_strstrcase (const char *haystack, const char *needle)
{
 ...
 if (haystack == ((void*)0)) return ((void*)0);
 if (needle == ((void*)0)) return ((void*)0);
 if (strlen (needle) == 0) return haystack;
 if (strlen (haystack) == 0) return ((void*)0);

 nuni = __builtin_alloca (sizeof (gunichar) * strlen (needle));
