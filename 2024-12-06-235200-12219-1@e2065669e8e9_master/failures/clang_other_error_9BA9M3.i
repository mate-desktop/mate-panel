# 1 "panel-applet-info.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 396 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "panel-applet-info.c" 2
# 24 "panel-applet-info.c"
# 1 "../config.h" 1
# 25 "panel-applet-info.c" 2

# 1 "./panel-applet-info.h" 1
# 27 "./panel-applet-info.h"
...
mate_panel_applet_info_new (const gchar *iid,
                            ...
                            gboolean wayland_supported)
{
 MatePanelAppletInfo *info = ((MatePanelAppletInfo*) g_slice_alloc0 (sizeof (MatePanelAppletInfo)));

 info->iid = g_strdup_inline (iid);
 info->name = g_strdup_inline (name);
 info->comment = g_strdup_inline (comment);
 info->icon = g_strdup_inline (icon);
 ...
 if (old_ids != ((void*)0)) {
  ...
  if ((len = g_strv_length ((gchar **) old_ids)) > 0) {
   int i;
   info->old_ids = ((gchar * *) g_malloc0_n ((len + 1), sizeof (gchar *)));
   for (i = 0; i < len; i++)
    info->old_ids[i] = g_strdup_inline (old_ids[i]);
  }
 }
 ...
 if (!x11_supported && !wayland_supported)
  g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "Applet %s has no supported platforms", name);
...
mate_panel_applet_info_free (gpointer data)
{
 ...
 do { if (1) g_slice_free1 (sizeof (MatePanelAppletInfo), (info)); else (void) ((MatePanelAppletInfo*) 0 == (info)); } while (0);
}
