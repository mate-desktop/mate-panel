...
panel_toplevel_update_applets_scale (gpointer user_data)
{
	...
	/* Use idle callback to restore size after the resize completes. */
	g_idle_add (panel_toplevel_restore_size, toplevel);
                                    ...
                                    gpointer   user_data)
{
	...
	if (new_scale != toplevel->priv->scale) {
		...
		/* Use idle callback to update applets after panel redraws */
		g_idle_add (panel_toplevel_update_applets_scale, toplevel);
	}
}
